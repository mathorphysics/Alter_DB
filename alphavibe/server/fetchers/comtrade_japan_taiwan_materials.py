"""
Japan → Taiwan Semiconductor Materials Flow
Tracks key materials exported from Japan to Taiwan:
  - Silicon Wafers  (HS 381800)  Shin-Etsu / Sumco → TSMC
  - Photoresist     (HS 370790)  JSR / Tokyo Ohka Kogyo → TSMC

Silicon wafer import volume is the hardest-to-fake proxy for TSMC's
real wafer-start rate — it is a direct physical constraint on output.

Mirrors the existing Japan → Korea materials tracker, extended to Taiwan.
...
Cached to: server/data/japan_taiwan_materials.json
"""
...
CACHE_PATH = DATA_DIR / "japan_taiwan_materials.json"
...
def fetch_japan_taiwan_materials(months: int = 24) -> List[Dict]:
    periods = _periods_yyyymm(months)
    merged: Dict[str, Dict] = {}

    for col_name, hs_code in HS_CODES.items():
        try:
            logger.info("Fetching UN Comtrade JP → TW HS %s (%s)", hs_code, col_name)
...
def load_japan_taiwan_materials() -> Optional[List[Dict]]:
    return json.loads(CACHE_PATH.read_text()) if CACHE_PATH.exists() else None


def get_japan_taiwan_materials() -> List[Dict]:
    cached = load_japan_taiwan_materials()
    if cached:
        return cached
    return fetch_japan_taiwan_materials()
