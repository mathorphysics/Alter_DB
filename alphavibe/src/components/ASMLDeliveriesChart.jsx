...
import { fetchASMLDeliveries } from '../api/altdata';
...
const SERIES = [
  { key: 'Taiwan', name: 'NL → TW', color: '#34d399' },
  { key: 'Korea',  name: 'NL → KR', color: '#467897' },
  { key: 'China',  name: 'NL → CN', color: '#f87171' },
  { key: 'USA',    name: 'NL → US', color: '#e7cd79' },
];
...
export default function ASMLDeliveriesChart({ compact = false }) {
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded flex items-center justify-center"
            ...
            <TrendingUp size={12} style={{ color: '#e7cd79' }} />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-[var(--fg)]">
              ASML Equipment Deliveries
            </h3>
            {!compact && (
              <p className="text-[11px] text-[var(--sub)]">
                NL HS-848620 exports to TW / KR / CN / US — USD Millions · UN Comtrade
              </p>
            )}
          </div>
          ...
          <span className="px-1.5 py-0.5 text-[9px] bg-[#e7cd79]/10 border border-[#e7cd79]/30 text-[#e7cd79] rounded font-medium">
            ASML
          </span>
          ...
          {RANGES.map((r, i) => (
            <button
              ...
              className={`px-2 py-0.5 text-[11px] rounded transition-all ${
                i === rangeIdx
                  ? 'bg-[#e7cd79]/20 text-[#e7cd79] font-medium'
        ...
        <div className="flex gap-4 mb-3 flex-wrap">
          {SERIES.map((s, i) => (
            <div key={s.key}>
              ...
              {i < SERIES.length - 1 && <div className="hidden sm:block w-px bg-[var(--border)] absolute" />}
            </div>
          )).reduce((acc, el, i) => {
            if (i > 0) acc.push(<div key={`sep-${i}`} className="w-px bg-[var(--border)]" />);
            acc.push(el);
            return acc;
          }, [])}
