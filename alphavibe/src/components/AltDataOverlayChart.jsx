...
const TICKER_STAGES = [
  ...
  },
  {
    ...
    tickers: [
      ...
      { value: '0981.HK',    label: 'SMIC',            sub: 'Mature Node' },
...
const ALT_CATALOG = [
  ...
  // ── AMAT / Lam Research / KLA / Onto / Nova (US equipment makers) ──────────
  {
    key:         'us-equip-tw',
    ...
    granularity: 'monthly',
    chartType:   'line',
    tickers:     ['AMAT', 'LRCX', 'KLAC', 'ONTO', 'NVMI', 'TSM'],
    color:       SERIES_COLORS[2],
  ...
  },
  {
    ...
    granularity: 'monthly',
    chartType:   'line',
    tickers:     ['AMAT', 'LRCX', 'KLAC', 'ONTO', 'NVMI', '005930.KS'],
    color:       SERIES_COLORS[5],
    fetch:       fetchUSEquipmentExports,
    extract:     (r) => (r.results ?? []).map((p) => ({ date: p.period, value: p.Korea ?? 0 })),
  },
  {
    ...
    granularity: 'monthly',
    chartType:   'line',
    tickers:     ['AMAT', 'LRCX', 'KLAC', 'ONTO', 'NVMI', '0981.HK'],
    color:       SERIES_COLORS[4],
    fetch:       fetchUSEquipmentExports,
    extract:     (r) => (r.results ?? []).map((p) => ({ date: p.period, value: p.China ?? 0 })),
  },

  // ── Shin-Etsu / TSMC — Japan → Taiwan materials ────────────────────────────
  {
    key:         'jp-tw-wafers',
  ...
  // ── Advantest / Teradyne — ATE shipments to OSAT hubs ─────────────────────
  {
    key:         'ate-jp-tw',
