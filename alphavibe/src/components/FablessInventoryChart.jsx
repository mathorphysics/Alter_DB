...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg px-3 py-2 shadow-xl text-xs">
      <div className="text-[var(--sub)] mb-1.5 font-medium">{label}</div>
      {SERIES.map(({ key, label: name, color }) => {
        ...
        return (
          <div key={key} className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-[var(--sub)]">{name}</span>
    ...
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div>
            <h3 className="text-sm font-semibold text-[var(--fg)]">Top Fabless Inventory Index</h3>
            <p className="text-[11px] text-[var(--sub)]">Quarterly inventory · Billions USD</p>
          </div>
          ...
          <LineChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
            ...
            <Legend
              ...
              formatter={(val) => {
                ...
                return <span style={{ color: 'var(--sub)' }}>{s?.label ?? val}</span>;
              }}
            />
            {SERIES.map(({ key, color }) => (
              <Line
                ...
                stroke={color}
                strokeWidth={1.5}
                dot={{ r: 3, fill: color, stroke: 'var(--bg)', strokeWidth: 1.5 }}
