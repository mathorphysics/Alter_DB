...
import { fetchJapanTaiwanMaterials } from '../api/altdata';
...
const SERIES = [
  { key: 'SiliconWafers', name: 'Silicon Wafers (HS 381800)', color: '#e7cd79' },
  { key: 'Photoresist',   name: 'Photoresist (HS 370790)',    color: '#a78bfa' },
];
...
export default function JapanTaiwanMaterialsChart({ compact = false }) {
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-[var(--fg)]">
              Japan → Taiwan Semiconductor Materials
            </h3>
            {!compact && (
              <p className="text-[11px] text-[var(--sub)]">
                Silicon wafers + photoresist exports to Taiwan — USD Millions · UN Comtrade
              </p>
            )}
          </div>
      ...
      {!compact && data && last && (
        <div className="flex gap-4 mb-3">
          {SERIES.map((s, i) => {
            ...
            return (
              <div key={s.key} className="flex items-center gap-4">
                ...
                <div>
                  <span className="text-[10px] text-[var(--sub)]">{s.key === 'SiliconWafers' ? 'Silicon Wafers' : 'Photoresist'}</span>
          ...
          <div>
            ...
            <div className="text-[11px] text-[var(--muted)]">UN Comtrade · HS 381800 / 370790</div>
          </div>
        </div>
      )}
