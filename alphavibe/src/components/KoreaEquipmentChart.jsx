...
import { fetchKoreaEquipmentInflow } from '../api/altdata';
...
export default function KoreaEquipmentChart({ compact = false }) {
  ...
  const nlDelta = last?.Netherlands && prev?.Netherlands
    ? ((last.Netherlands - prev.Netherlands) / prev.Netherlands * 100).toFixed(1)
    : null;
  const usDelta = last?.USA && prev?.USA
    ? ((last.USA - prev.USA) / prev.USA * 100).toFixed(1)
    : null;
    ...
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded flex items-center justify-center"
               style={{ background: '#467897' + '20', border: '1px solid ' + '#467897' + '40' }}>
            <TrendingUp size={12} style={{ color: '#467897' }} />
          </div>
          <div>
            <h3 className="text-sm font-semibold text-[var(--fg)]">
              Korea HS-848620 Equipment Inflow
            </h3>
            {!compact && (
              <p className="text-[11px] text-[var(--sub)]">
                Semiconductor equipment exports to South Korea — USD Millions · UN Comtrade
              </p>
            )}
          </div>
      ...
      {!compact && data && last && (
        <div className="flex gap-4 mb-3">
          <div>
            <span className="text-[10px] text-[var(--sub)]">NL → KR</span>
            <div className="text-sm font-mono font-semibold text-[#467897]">
              ${last.Netherlands?.toLocaleString('en-US', { maximumFractionDigits: 1 })}M
              {nlDelta && (
                <span className={`ml-1.5 text-[11px] ${Number(nlDelta) >= 0 ? 'text-[#467897]' : 'text-red-400'}`}>
                  {Number(nlDelta) >= 0 ? '+' : ''}{nlDelta}% MoM
                </span>
              )}
            </div>
          </div>
          ...
          <div>
            <span className="text-[10px] text-[var(--sub)]">US → KR</span>
            <div className="text-sm font-mono font-semibold text-[#e7cd79]">
              ${last.USA?.toLocaleString('en-US', { maximumFractionDigits: 1 })}M
              {usDelta && (
                <span className={`ml-1.5 text-[11px] ${Number(usDelta) >= 0 ? 'text-[#e7cd79]' : 'text-red-400'}`}>
                  {Number(usDelta) >= 0 ? '+' : ''}{usDelta}% MoM
                </span>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="text-[11px] text-[var(--muted)]">UN Comtrade · HS 848620</div>
          </div>
        </div>
      )}
